...
Shader "MapsSDK/ExaggeratedClippingVolume"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "..\..\..\Packages\Microsoft.Maps.Unity.0.6.1\lib\unity\Map\SupportingScripts\Shaders\MRTKIntegration-MapsSDK.cginc"
            ...
            v2f vert(appdata v)
            {
                ...
                // Make the volume larger as required 
                float3 exaggeratedVertex = v.vertex;
                exaggeratedVertex.y *=  max(1, _ElevationScale);

                o.pos = UnityObjectToClipPos(exaggeratedVertex);

                float3 worldPosition = mul(unity_ObjectToWorld, float4(exaggeratedVertex.xyz, 1.0)).xyz;
#if ENABLE_MRTK_INTEGRATION
